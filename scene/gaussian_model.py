...
from sunfaded.networks import LightMLP
...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int):
        ...
        self._env = torch.empty(0)
        self.denom = torch.empty(0)
        self.optimizer = None
        self.percent_dense = 0
        self.spatial_lr_scale = 0
        self.mlp = LightMLP().cuda()
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        albedo = torch.zeros((pcd.points.shape[0], 7)).float().cuda()
        albedo[:, :3] = torch.tensor(np.asarray(pcd.colors)).float().cuda()
        albedo[:, 6:] = 10
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        features = (
            torch.zeros((albedo.shape[0], 3, (self.max_sh_degree + 1) ** 2))
            .float()
            .cuda()
        )
        features[:, :3, 0] = 1.0
        ...
        dist2 = torch.clamp_min(
            distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 1e-07
        )
        ...
        self._env = nn.Parameter(
            torch.zeros((3, 1, 1), device="cuda").requires_grad_(True)
        )
        self._offset = nn.Parameter(
            torch.zeros((5, 1, 1), device="cuda").requires_grad_(True)
        )
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {"params": [self._env], "lr": training_args.env_lr, "name": "env"},
            ...
            {"params": [self._offset], "lr": training_args.env_lr, "name": "offset"},
        ]
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate(
            (xyz, normals, f_dc, f_rest, opacities, scale, rotation, albedo), axis=1
        )
